...
pub fn build_map(
    ...
    let codomain_len = (keys.len() + free.len()) as u32;
    ...
    TokenStream::from(quote! {{
       ...
       if (#min_key_size..=#max_key_size).contains(&__key.len()) {
            let key_hash = __key.iter().fold(#seed, |h, &c| {
                h.wrapping_mul(0x01000193).wrapping_add(#c as u32)
            });
            let pilot_hash = (PILOTS_TABLE[key_hash as usize % #pilots_len] as u32).wrapping_mul( 0x9e3779b9);
...
#[inline]
pub fn hash_key(key: &[u8], seed: u32) -> u32 {
...
#[inline]
pub fn hash_pilot_value(pilot_value: u16) -> u32 {
    (pilot_value as u32).wrapping_mul(0x9e3779b9)
}

#[inline]
pub fn get_bucket(key_hash: u32, buckets: u32) -> usize {
    (key_hash % buckets) as usize
}

#[inline]
pub fn get_index(key_hash: u32, pilot_hash: u32, codomain_len: u32) -> usize {
...
pub fn generate_phf(keys: Vec<(Key, Option<Expr>)>) -> Phf {
    ...
    let codomain_len = {
        let candidate = (n as f64 / alpha).ceil() as u32;
...
fn try_generate_phf(
    ...
    assert!((entries.len() as u32) < u32::MAX);
