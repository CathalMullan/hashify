...
fn lookup_words_big(key: &[u8]) -> Option<&Word> {
    hashify::map! { key, Word,
    "addflag" => Word::AddFlag,
    "addheader" => Word::AddHeader,
    "address" => Word::Address,
    "addresses" => Word::Addresses,
...
fn lookup_charsets_big(key: &[u8]) -> Option<&u32> {
    hashify::map! {
    key, u32,
    "koi8_r" => 35,
    "windows_1253" => 97,
...
fn lookup_entities_big(key: &[u8]) -> Option<&u32> {
    hashify::map! { key, u32,
    "AElig" => 198,
    "AMP" => 38,
    "Aacute" => 193,
...
fn bench_hashmaps(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("hashmap_words");

    group.bench_function("phf_tiny_words", |b| {
        b.iter(|| {
            for key in &keys {
    ...
    group.bench_function("hashify_tiny_words", |b| {
        b.iter(|| {
            for key in &keys {
    ...
    group.bench_function("hashify_large_words", |b| {
        b.iter(|| {
            for key in &keys {
                lookup_words_big(key.as_bytes()).unwrap();
            }
        })
    });
    ...
    let mut group = c.benchmark_group("hashmap_charsets");

    group.bench_function("phf_tiny_charsets", |b| {
        b.iter(|| {
            for key in &keys {
    ...
    group.bench_function("hashify_tiny_charsets", |b| {
        b.iter(|| {
            for key in &keys {
    ...
    group.bench_function("hashify_large_charsets", |b| {
        b.iter(|| {
            for key in &keys {
                lookup_charsets_big(key.as_bytes()).unwrap();
            }
        })
    });
    ...
    let keys: Vec<String> = ENTITIES.keys().map(|s| s.to_string()).collect();

    println!("Entities: {:?}", ENTITIES.len());

    let mut group = c.benchmark_group("hashmap_entities");

    group.bench_function("phf_large_entities", |b| {
        b.iter(|| {
            for key in &keys {
                ENTITIES.get(key).unwrap();
            }
        })
    });

    group.bench_function("hashify_large_entities", |b| {
        b.iter(|| {
            for key in &keys {
                lookup_entities_big(key.as_bytes()).unwrap();
            }
        })
    });
